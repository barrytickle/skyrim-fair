...
| "Whiterun Fair" welcome sign | JeffK (jeffkolada, Sketchfab) | bundled, converted | Sketchfab Standard; **cleared by Barry to ship (2026-09-25)**, with attribution |
...
**Licence: cleared to ship** (Barry, 2026-09-25: "the welcome sign is okay").
- Sketchfab lists the model under its Standard licence, and the author's own description
  says it's "free to download and use in your works with CC attribution".
- Barry has confirmed it can ship in the fair, with the attribution below. It's in release
  packages from 1.0 (`tools/package.py` no longer reserves it).
- Its source files stay git-ignored, as before.
...
**Source:** generated by Barry with **ElevenLabs** on its Pro plan (2026-09-25), whose
paid plans include a commercial licence for the audio. The sources are kept out of git.
...
projects (checked by Barry, 2026-09-25), but not to redistribute as standalone animation
files. The fair ships the converted clip inside the mod, as Skyrim mods commonly do
(Professional Dancer, for one, credits Mixamo).
