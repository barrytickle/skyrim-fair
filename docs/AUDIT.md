...
## Current pass: market avenue, Traders' Crossing and trading rows (2026-09-22, night)

This is a structural market-layout pass: stall **shells** and their arrangement, with only
enough dressing to show the lanes. There are no merchants, NPCs, navmesh, systems or
final themed merchandise. Generated by `FairMarket.cs` from `fairWorld.market`.
Everything else in the plugin is unchanged apart from the new records.

Previews from the written plugin: `docs/images/market_plan.png` (plan view: the stage,
wall, zones, keep-outs in red, lanes in blue) and `docs/images/market_views.png` (from
the gate up the avenue, and from the crossing down the West Branch). The renderer
draws canvas the same flat brown as wood.

### Result

**39 stall shells, 159 pieces, 39 named shell markers, 4 crossing banner posts.**

| Zone / lane | Stalls | Character |
| --- | --- | --- |
| **Main avenue** (gate to crowd square) | 15 + 4 back-to-back | lined both sides, alternating pinches (480-520 between stall fronts) and pockets, gently meandering; the rival pair faces across a pinch just north of the crossing; the sweetroll stall sits near the crowd square |
| **Traders' Crossing** | (avenue) | the avenue widens to 1,300 around (1992, 1000), with no stalls in its mouths; banner posts mark its four corners |
| **West Branch** (the main trading row) | 4 | runs from the crossing to near the west wall, crossing the back and outer lanes as T-junctions |
| **East Branch** | 1 | a short spur toward the archery range, which stays clear |
| **West back lanes** (south and north of the branch) | 10 + 1 back-to-back | a secondary north-south lane through the market block |
| **West outer lanes** | 4 | east-facing row inside the west wall |
| **Alleys** (two west, one east) | 0 | **cut-through passages** only: they keep their corridor clear and make the mini-lanes and pockets between blocks |

Shell themes handed out (each marker is `SkyrimFairStall<Theme><NN>`, a persistent
`XMarkerHeading` at the stall front facing in, e.g. `SkyrimFairStallImperial01`):

- **Rival and signature stalls**: imperial, stormcloak, sweetroll
- **Food and drink** near the avenue ends: food, drink, sweets, pies, cheese, roast,
  produce, bakery
- **Specialists** in the west rows: mage, dwemer, books, alchemy, jewellery, elven,
  scrolls, curios, herbs, candles, pottery, furs, toys
- **Trades** at the edges: hunter, leather, rugs, timber, hay, bone, storage, tanner,
  cooper, weaver, potter

These are slots for later merchant identities, not dressing.
...
The condense cost the market only a little. The west block lost about 400 of width, so
one outer lane had to become a single row. Most of the early sparseness was layout-model
problems, now fixed:
- circular corridors spilled pocket widths sideways
- stalls measured their set-back at one point
- crossing lanes blocked each other

Fixing those took it from 16 stalls to 39.
...
- Build clean. Generator run twice: identical SHA256 **`c0e25d02...`** (404,261 bytes).
  Deployed byte-identical.
- Market meshes checked vertex by vertex against the plugin:
  - 0 outside the compound, and the nearest is 424 from the wall line
  - 0 in the crowd square, the stage zone, the gate-to-stage sightline band and the
    archery range
  - One crate (`CrateSmallLong01` at 1236, -984) reaches 36 onto the entrance
    forecourt's paint. That is harmless and has been left.
- **Not verified in game**: walkability between stalls (no navmesh yet), stall
  collision, and how the canvas reads.

### What Barry should test in game

1. From the gate, walk the avenue. Does it feel like a dense market street with pinches
   and pockets, and does the stage stay the anchor?
2. Is the Traders' Crossing recognisable, with the West Branch opening off it?
3. Wander the west block's back lanes and passages. Is it a controlled maze, or too
   sparse or too tight?
4. Do the rival stalls read as facing each other across the street
   (`player.moveto SkyrimFairStallImperial01`)?
5. Any stall floating, clipping another, or blocking a lane?
...
| Size | 404,261 bytes (with the market; 390,425 before it) |
| SHA256 | `c0e25d02fa907c5404f9a1fb910f6c8a5de4ba9e29f111ab371e2e6d8ffc2894` |
| Deployed | byte-identical at `E:\Modlists\Still In Skyrim\mods\Skyrim Fair\SkyrimFair.esp` (2026-09-22, night; replaced `092515db...`, the condensed build) |
