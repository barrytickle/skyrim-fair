...
- **crowd density is tiered** (`crowds.tiers`, built last): each tier on a persistent enable-parent marker, switched live by the stage script from `SkyrimFairCrowdTier` (0..n). Tier 2's wanderers sandbox (`DefaultSandboxEditorLocation512`) on the navmesh. Crowded Streets (audited) spawns and deletes crowds by script, which suits huge worlds; for the fair, placed tiers are better
