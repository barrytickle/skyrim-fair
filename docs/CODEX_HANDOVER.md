...
- **the musicians and the crowd's moves live in `performers.config.json`** (`audio.stage.performersFile`, loaded in Program.cs into `Stage.Band/Orchestra/BandStop/BandPackage/DrumIdle` and `Crowds.Dance/Clap/CheerIdles` and their lengths); a musician's `idle` may be an `instruments` name (`lute`, `drum`, `flute`)
- **the stage songs live in `songs.config.json`** (`audio.stage.songsFile`; the generator, `build_audio.py` and `build_sections.py` all read it there). **Song sections** (`sections`, `[start, drums, crowd]`) drive the drummers (rest when calm) and the dancers (dance / clap / cheer). Seed them from the stems with `tools/bards/build_sections.py --write`; after that they're hand-tuned, so only `--force` overwrites. Idle clip lengths come from the archive through the vendored HKX codec (`character-actors/folk-dance/vendor`)
