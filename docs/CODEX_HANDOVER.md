...
- **per-NPC spells from other mods are switched off at the fair by compatibility patches** (`compatPatches`, `FairPatches.cs`):
  - Each is a light plugin that overrides the spell with `GetGlobalValue SkyrimFairAtFair == 0` (and `GetInWorldspace SkyrimFairWorld == 0`) first on every effect. The stage script keeps the global at 1 while the player is at the fair.
  - `GetInWorldspace` alone didn't hold in game. The fair's worldspace has Tamriel as its parent; use the global.
  - Don't try to fix such a clash in Papyrus. The flood starts at load, and the fix's events wait in the same queue (the retired `SkyrimFairNpcGuard` never got to run).
  - To add a patch, name the mod's plugin and spells in `compatPatches`.
