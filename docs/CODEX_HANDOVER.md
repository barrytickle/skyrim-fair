...
- the archers also have `SkyrimFairArcherHoldPackage` above the training package, live while the global `SkyrimFairArcherHold` is 1: the stage script flips it on and off on arrival and load, to restart the training package from the top (EvaluatePackage alone keeps a running package)
