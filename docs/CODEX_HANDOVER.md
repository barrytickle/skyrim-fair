...
- **dance styles** (`songs.config.json` `danceStyles`, `tools/bards/build_dances.py`): Professional Dancer's clips replace the Cicero dance through OAR per dancer, picked by actor value `Variable10` (index 77) the script sets before `PlayIdle`. The clips are Mixamo's: never commit or ship them, build them from the user's install
- **fireworks** (`songs.config.json` `fireworks`): Fireworks.esp is a soft dependency. The script looks launchers up with `Game.GetFormFromFile` and `PlaceAtMe`s them at the generated site markers as each song ends. Use only the light ones (mortars, flare): the star bursters and rockets spawn 120-200 scripted lights each; never the Bunger or Flame Jet (they cast fire)
