...
- before deploying, generate the physics-free props (the market's goods and the towers' lantern) with `python tools/make_static_props.py --data "<stock Data>"`: it reads `tools/static_props_sources.json`, writes `assets/meshes/SkyrimFair/Props/` and `TowerLantern.nif` (modified vanilla meshes, git-ignored, shipped only in the built mod; the tool makes rigid bodies fixed and must never null a collision link, which crashes the game) and refreshes `tools/static_props.json` (committed bounds; the generator makes `SkyrimFairProp<Name>` STATs from it, `@Prop<Name>` in modules); deploy `meshes\SkyrimFair\` with the ESP
