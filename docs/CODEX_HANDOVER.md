...
- invisible walls are `fairWorld.collisionWalls` segments, built as vanilla `CollisionMarker` box primitives (half-extents). **Put them on layer 3 (L_TRANSPARENT)**: on the default layer SkyParkour (in Barry's list) mantles onto their tops. Transparent blocks the player and NPCs and isn't a climbable ledge
