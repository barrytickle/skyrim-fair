...
- **the archers don't resume their training package after a load** in a world with no navmesh; the stage script sets each on its stand (`MoveTo` the unkeyed linked ref) and calls `EvaluatePackage` on every arrival and load. Navmesh is the real fix
