...
- **a run-up (force greet)**: copy a vanilla ForceGreet-template package (`dunBlindCliffForceGreetAndRewardPlayer` has every location on the player: he runs straight to them), set its topic input, and gate it by condition; one aimed at the player needs "the player is in the fair's worldspace" too, or the NPC sets out after them across Skyrim. Its topic starts a Blocking branch (Player category), as Ancano's in MG03. A topic in the cameos quest with no EditorID keeps the proven voice name `skyrimfaircameos__<info>_<response>`; a line of two responses gets `_1` and `_2` (`FairPassport.cs`)
